...
const Dashboard = () => {
    ...
    const fetchData = async () => {
        try {
            // Fetch rooms
            const roomsResponse = await axios.get(`${ROOM_BASE_URL}/get-room`);
    ...
    const processChartData = (roomsData, bookingsData) => {
        ...
        const revenueData = last7Days.map(date => {
            const dayBookings = bookingsData.filter(booking => 
                booking.Check_in.split('T')[0] === date
            );
        ...
        const notavailableRooms = roomsData.filter(room => room.Status === "Unavailable").length;
        const occupancyPercentage = (notavailableRooms / totalRooms) * 100;
        setOccupancyData([
            { name: 'Unavailable', percentage: occupancyPercentage },
            { name: 'Available', percentage: 100 - occupancyPercentage }
        ]);
